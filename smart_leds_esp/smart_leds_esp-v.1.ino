...
#define NUM_LEDS    160//total number of leds in the main strip ->  80cm/0,5cm = 160leds
...
int main_led_map[] = {80,80};//number of leds per section: 0-if section is off; 1- if section is on
...
///////////////////////////////////////////////////////
//....LOWER CABINET LED STRIP INITIALIZATION.........//
///////////////////////////////////////////////////////
#define LED_PIN_3    3//?????????????????????????????????????????
#define NUM_LEDS_3   120//total number of leds 
#define NUM_SEC_3     1 //total number of led sections

CRGB leds_3[NUM_LEDS_3];//led strip controller
   ...
   if(strcmp(topic, topicShelf1)==0){//led shelves topic
    ...
    if(strcmp(topic,acdTopic) == 0){
     //blinking leds orange and yellow
      ...
      if(strcmp(payloadChar, "Opening")==0){
       
        for(int i = 0; i < NUM_LEDS_3; i = i+2){
          leds_3[i] = CRGB::Yellow;
        }
        FastLED.show();
        delay(500);
        for(int i = 0; i < NUM_LEDS_3; i = i+2){
        ...
        FastLED.show();
        delay(500);      
        for(int i = 1; i < NUM_LEDS_3; i = i+2){
          leds_3[i] = CRGB::Orange;
        }
        FastLED.show();
        delay(500);
        for(int i = 1; i < NUM_LEDS_3; i = i+2){
      ...
      if(strcmp(payloadChar, "Closing")==0){
...
void setup() {
 ...
 FastLED.addLeds<WS2812,LED_PIN_3 , GRB>(leds_3, NUM_LEDS_3); //lower cabinets led strip
  Serial.print("\n\r Initializing Wifi \n\r");
